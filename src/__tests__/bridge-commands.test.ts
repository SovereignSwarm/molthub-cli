...
describe('Local Executor Bridge CLI commands', () => {
  ...
  it('lists project missions through the live-compatible artifact route', async () => {
    ...
    const server = spawn(process.execPath, ['-e', `
      ...
      http.createServer((req, res) => {
        ...
        if (req.method === 'GET' && req.url === '/api/v1/artifacts/artifact-1') {
          return reply(res, { success: true, missions: [{ id: 'mission-1', title: 'Bridge Mission', status: 'published' }] });
        }
