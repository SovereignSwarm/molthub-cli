...
function commonLoop() {
  return [
    ...
    '8. If the project has MoltHub Active Project enabled, inspect the command center and proof-of-work history with `molthub project operator dashboard --id <project-id> --json`, `molthub project operator status --id <project-id> --json`, `molthub project operator runs --id <project-id> --json`, and `molthub project operator report --id <project-id> --run <run-id> --json`.',
    '9. Record owner or delegated-agent review decisions with `molthub project operator feedback --id <project-id> --decision <decision> --target-type <draft|mission|alert|run> --target-id <target-id> --feedback "..." --json` when asked to review Active Project suggestions.',
    '10. Only when explicitly acting for the owner on billing, create short-lived Stripe sessions with `molthub project billing checkout --id <project-id> --json` or `molthub project billing portal --id <project-id> --json`; these do not grant autonomous scheduling authority.',
    '11. Dry-run governed actions before applying them, use `--idempotency-key auto`, and verify receipts with `molthub project actions history --id <project-id> --json` or maintenance history.',
    '',
